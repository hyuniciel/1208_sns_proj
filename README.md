...
### 🗄️ 데이터베이스 통합
- Supabase 공식 Next.js 패턴 지원 (`@supabase/ssr`)
- Clerk 토큰 기반 Supabase 인증 (JWT 템플릿 불필요)
- 환경별 Supabase 클라이언트 분리:
  - Supabase 자체 인증: `createClient()` (Server Component)
  - Clerk 통합: `createClerkSupabaseClient()` (Server), `useClerkSupabaseClient()` (Client)
  - 공개 데이터: `supabase` (Client Component)
  - 관리자 권한: `getServiceRoleClient()` (Server only)
- SQL 마이그레이션 시스템
