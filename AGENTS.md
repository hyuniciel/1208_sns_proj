...
- **Authentication**: Clerk (with Korean localization - `koKR` from `@clerk/localizations`)
...
- `lib/supabase/clerk-client.ts`: Client Component용 Supabase 클라이언트 (권장)
- `lib/supabase/server.ts`: Server Component/Action용 Supabase 클라이언트 (권장)
- `lib/supabase.ts`: 레거시 파일 (deprecated, 하위 호환성용)
